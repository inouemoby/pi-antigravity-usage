...
Displays real-time Google Antigravity 5-hour rolling limits and weekly quota in Pi's footer. Also provides the `/antigravity` command and `antigravity_usage` tool for instant quota inspection.
...
- **`/antigravity` Command**: Visual progress bar chart with exact usage, remaining quota %, and human-readable countdowns to reset.
- **`antigravity_usage` Tool**: Registered LLM tool allowing the agent to check remaining quota when asked.
...
The extension registers `antigravity_usage` so you can ask Pi:
- *"How much quota do I have left on Antigravity?"*
- *"When does my Gemini 5-hour limit reset?"*
